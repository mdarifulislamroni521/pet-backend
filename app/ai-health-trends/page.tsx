...
export default function AIHealthTrendsPage() {
  ...
  useEffect(() => {
    const fetchRealData = async () => {
      ...
      try {
        // Fetch patients
        const patientsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8989"}/api/patients`);
        let patients: any[] = [];
        ...
        // Fetch appointments
        const appointmentsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8989"}/api/appointments`);
        ...
        // Fetch AI results
        const aiResultsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8989"}/api/ai-results/debug`);
        let aiResults: any[] = [];
