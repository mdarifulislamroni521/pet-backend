...
export default function PatientViewPage() {
  ...
  const fetchAIResults = async () => {
    ...
    try {
      ...
      const debugResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8989"}/api/ai-results/debug?patientId=${patientId}`);
      if (debugResponse.ok) {
      ...
      // Then fetch normally
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8989"}/api/ai-results?patientId=${patientId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleDeleteResult = async (resultId: string, resultTitle: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8989"}/api/ai-results?id=${resultId}`, {
        method: 'DELETE',
      });
